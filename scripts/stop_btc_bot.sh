...
if [[ ! -f "$PID_FILE" ]]; then
  echo "BTC bot PID file not found; checking running processes by pattern."
else
  PID="$(cat "$PID_FILE")"
  ...
  if kill -0 "$PID" 2>/dev/null; then
    ...
    echo "BTC bot is not running for PID file entry. Removing stale PID file."
  fi
fi
...
for extra_pid in "${EXTRA_PIDS[@]}"; do
  if kill -0 "$extra_pid" 2>/dev/null; then
    kill "$extra_pid"
    echo "BTC bot stopped (pattern match). PID=$extra_pid"
    stopped_any=true
  fi
done

if [[ "$stopped_any" = false ]]; then
  echo "No running BTC bot process found."
fi